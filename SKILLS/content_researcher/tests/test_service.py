...
from SKILLS.content_researcher.schemas import ContentResearchInput
from SKILLS.content_researcher.service import (
...
async def test_run_returns_aggregated_output(skill: ContentResearcherSkill, ctx: SkillContext):
    ...
    with (
        patch("SKILLS.content_researcher.cache.get_cached", new_callable=AsyncMock, return_value=None),
        patch("SKILLS.content_researcher.cache.set_cached", new_callable=AsyncMock),
        patch("SKILLS.content_researcher.api_client.search_tiktok", new_callable=AsyncMock, return_value=_TIKTOK_RAW),
        patch("SKILLS.content_researcher.api_client.search_pinterest", new_callable=AsyncMock, return_value=_PINTEREST_RAW),
        patch("SKILLS.content_researcher.service.get_supabase") as mock_supa,
    ):
...
async def test_run_returns_cached_result(skill: ContentResearcherSkill, ctx: SkillContext):
    ...
    with (
        patch("SKILLS.content_researcher.cache.get_cached", new_callable=AsyncMock, return_value=cached_output),
        patch("SKILLS.content_researcher.api_client.search_tiktok", new_callable=AsyncMock) as mock_tt,
    ):
        result = await skill.run(input_data, ctx)
