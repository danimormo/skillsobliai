"""ScrapeCreators API client for TikTok and Pinterest content search."""
...
async def search_tiktok(
    ...
    """Search TikTok organic videos by keyword via ScrapeCreators."""
    params: dict[str, Any] = {
        "keyword": keyword,
        "region": region,
        "limit": limit,
    }
    url = f"{_base_url()}/v1/tiktok/search"
...
async def search_pinterest(
    ...
    """Search Pinterest pins by keyword via ScrapeCreators."""
    params: dict[str, Any] = {
        "keyword": keyword,
        "region": region,
        "limit": limit,
    }
    url = f"{_base_url()}/v1/pinterest/search"

    async def _do(c: httpx.AsyncClient) -> list[dict[str, Any]]:
        data = await _request_with_retry(c, "GET", url, params=params, headers=_headers())
        return data.get("pins", data.get("data", []))
