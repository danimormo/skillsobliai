...
from SKILLS.landing_builder.schemas import LandingBuilderInput, LandingBuilderOutput
from SKILLS.landing_builder.service import LandingBuilderSkill

logger = logging.getLogger(__name__)

router = APIRouter(tags=["landing-builder"])

_skill = LandingBuilderSkill()
...
@router.post("/run", response_model=SkillResult[LandingBuilderOutput])
async def run_landing_builder(
    body: LandingBuilderInput,
    authorization: str = Header(...),
) -> SkillResult[LandingBuilderOutput]:
    """Generate landing page copy for a product."""
