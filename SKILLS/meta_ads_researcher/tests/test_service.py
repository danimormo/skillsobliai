...
from SKILLS.meta_ads_researcher.schemas import MetaAdsResearchInput
from SKILLS.meta_ads_researcher.service import MetaAdsResearcherSkill
...
async def test_run_happy_path(skill, ctx, sample_input):
    ...
    with (
        patch(
            "SKILLS.meta_ads_researcher.cache.get_cached",
            new_callable=AsyncMock,
            return_value=None,
        ),
        patch(
            "SKILLS.meta_ads_researcher.cache.set_cached",
            new_callable=AsyncMock,
        ),
        patch(
            "SKILLS.meta_ads_researcher.api_client.fetch_ads",
            new_callable=AsyncMock,
            return_value=_make_api_response(),
        ),
        patch(
            "SKILLS.meta_ads_researcher.service.get_supabase",
        ) as mock_sb,
    ):
