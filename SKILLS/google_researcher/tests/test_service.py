...
from SKILLS.google_researcher.schemas import GoogleResearchInput
from SKILLS.google_researcher.service import GoogleResearcherSkill
...
async def test_run_returns_rising_trend():
    ...
    with (
        patch("SKILLS.google_researcher.service.cache.get_cached", new_callable=AsyncMock, return_value=None),
        patch("SKILLS.google_researcher.service.cache.set_cached", new_callable=AsyncMock),
        patch("SKILLS.google_researcher.service.api_client.fetch_google_trends", new_callable=AsyncMock, return_value=_fake_trends([20, 25, 30, 30, 35, 60, 70, 80, 85, 90])),
        patch("SKILLS.google_researcher.service.api_client.search_reddit", new_callable=AsyncMock, return_value=_fake_reddit_posts()),
        patch("SKILLS.google_researcher.service.get_supabase") as mock_sb,
    ):
...
async def test_run_returns_cached_result():
    ...
    with (
        patch("SKILLS.google_researcher.service.cache.get_cached", new_callable=AsyncMock, return_value=cached),
        patch("SKILLS.google_researcher.service.api_client.fetch_google_trends", new_callable=AsyncMock) as mock_trends,
    ):
        result = await SKILL.run(inp, ctx)
...
async def test_run_computes_sentiment_and_demand_score():
    ...
    with (
        patch("SKILLS.google_researcher.service.cache.get_cached", new_callable=AsyncMock, return_value=None),
        patch("SKILLS.google_researcher.service.cache.set_cached", new_callable=AsyncMock),
        patch("SKILLS.google_researcher.service.api_client.fetch_google_trends", new_callable=AsyncMock, return_value=_fake_trends([50] * 10)),
        patch("SKILLS.google_researcher.service.api_client.search_reddit", new_callable=AsyncMock, return_value=posts),
        patch("SKILLS.google_researcher.service.get_supabase") as mock_sb,
    ):
        mock_sb.return_value = MagicMock()
        mock_sb.return_value.table.return_value.insert.return_value.execute.return_value = None
